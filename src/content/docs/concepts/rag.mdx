...
const vectorResults = await vectorRAG.search({
...
const documentResults = await documentRAG.search({
...
const filteredResults = await vectorRAG.search({
...
const recentResults = await vectorRAG.search({
  ...
  async hybridSearch(query: string, limit: number = 5) {
    ...
    documentResults.forEach(result => {
      if (combinedResults.has(result.id)) {
        combinedResults.get(result.id).documentScore = result.score;
      } else {
        combinedResults.set(result.id, {
          ...result,
          vectorScore: 0,
          documentScore: result.score
        });
      }
    });
