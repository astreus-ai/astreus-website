...
Tasks enable agents to break down complex problems into manageable sub-tasks, execute them in a specific order, and coordinate the results. Each task automatically creates its database table when first created.
...
Tasks are created through the TaskManager and automatically create their database table:
