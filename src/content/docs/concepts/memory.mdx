...
Memory in Astreus 0.1.9+ is **agent-centric** and provides powerful conversation history management with **session-based organization**. Instead of managing memory separately, you access it directly through agents with built-in session tracking.

## Agent-Centric Memory Architecture

With the new approach, memory is accessed through agents and automatically manages:

- **Session-Based History**: Organized conversations by session ID
- **Automatic Message Storage**: Messages saved during `agent.chat()` and `agent.streamChat()`
- **Direct Memory Access**: Full memory operations through `agent.config.memory`
- **Session Management**: Built-in session listing and management
- **Metadata Support**: Rich metadata for each message and session
