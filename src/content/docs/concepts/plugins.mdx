...
import { createResendPlugin, createWhatsAppPlugin, createXPlugin } from '@astreus-ai/plugins';
...
async function createAgentWithPlugins() {
  ...
  const resendPlugin = createResendPlugin({
  ...
  agent.addTool(whatsappPlugin.getTool());
  agent.addTool(xPlugin.getTool());
...
```typescript
import { createResendPlugin } from '@astreus-ai/resend-plugin';

const resendPlugin = createResendPlugin({
  apiKey: process.env.RESEND_API_KEY,
  fromEmail: 'support@company.com',
  fromName: 'Company Support' // Optional
});
...
```typescript
import { createWhatsAppPlugin } from '@astreus-ai/whatsapp-plugin';
...
```typescript
import { createXPlugin } from '@astreus-ai/x-plugin';
...
const hasEmailTool = availableTools.some(tool => tool.name === 'send_email');
console.log('Email capability:', hasEmailTool ? 'Available' : 'Not available');

// Get tool by name
const emailTool = availableTools.find(tool => tool.name === 'send_email');
if (emailTool) {
  console.log('Email tool description:', emailTool.description);
  console.log('Email tool parameters:', emailTool.parameters);
}
```
