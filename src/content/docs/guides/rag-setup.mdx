...
CREATE TABLE knowledge_base_chunks (
  ...
  chunkIndex INTEGER,
  metadata JSON,
  FOREIGN KEY (documentId) REFERENCES knowledge_base(id)
);
...
CREATE TABLE knowledge_base_embeddings (
  id VARCHAR PRIMARY KEY,
  chunkId VARCHAR,
  embedding VECTOR, -- or JSON for SQLite
  FOREIGN KEY (chunkId) REFERENCES knowledge_base_chunks(id)
);

-- Performance indexes
CREATE INDEX idx_chunks_document ON knowledge_base_chunks(documentId);
CREATE INDEX idx_embeddings_chunk ON knowledge_base_embeddings(chunkId);
```
...
-- Text chunks for keyword search
...
CREATE INDEX idx_chunks_content ON documents_chunks USING gin(to_tsvector('english', content));
```
